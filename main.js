...
// ════════════════════════════════════════════════════════════
// CSS is loaded via <link rel="stylesheet"> in index.html
...
function startMatch(playerChar, aiChar) {
  ...
  async function showSettings() {
    state = 'settings'
    const screen = await awaitCreateSettingsScreen()
    showScreen(screen)
  }

  function awaitCreateSettingsScreen() {
    // Lazy import to avoid top-level cycles
    return import('./src/ui/screens.js').then((m) => m.createSettingsScreen(() => showTitle()))
  }
  currentMatch.start()
}
...
// Démarrage — précharge les assets et active l'overlay dev
DevOverlay.install()
const assetList = CHARACTERS.map((c) => `/models/${c.id}.glb`)
Loader.preload({
  assets: assetList,
  sounds: [],
}).then(() => {
  ...
  // Même en cas d'erreur de précharge, démarrer pour permettre debug
  Settings.applyToAudio(audio)
  showTitle()
})
